...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['calculate'])) {
    ...
    // Insert calculated marks into internal_marks table
    if ($student_name !== "No student found with this Roll Number." && $internal_marks != "Data Missing or Invalid") {
        $sql_insert = "INSERT INTO internal_marks (roll_num, student_name, sem, subject, internal_marks) 
                       VALUES (?, ?, ?, ?, ?)";
        $stmt_insert = $conn->prepare($sql_insert);
        $stmt_insert->bind_param("ssisd", $roll_num, $student_name, $sem, $sub, $internal_marks);

        if ($stmt_insert->execute()) {
            echo "<p class='success'>Internal Marks Saved Successfully!</p>";
        } else {
            echo "<p class='error'>Error: " . $stmt_insert->error . "</p>";
        }
        $stmt_insert->close();
    } else {
        echo "<p class='error'>Invalid data. Could not save internal marks.</p>";
    }
}
...
<body>
    ...
    <div class="form-container">
        <form method="post" action="">
            ...
            <input type="submit" name="calculate" value="Calculate and Save Internal Marks">
        </form>
    </div>
